...
export class HeaderAuthProvider implements core.AuthProvider {
    ...
    public static canCreate(options: HeaderAuthProvider.Options): boolean {
        return options.apiKey != null || process.env?.RESPEECHER_API_KEY != null;
    }

    public async getAuthRequest(_arg?: { endpointMetadata?: core.EndpointMetadata }): Promise<core.AuthRequest> {
        const apiKey = (await core.Supplier.get(this.headerValue)) ?? process.env?.RESPEECHER_API_KEY;
        if (apiKey == null) {
            throw new errors.RespeecherError({
                message:
                    "Please specify a apiKey by either passing it in to the constructor or initializing a RESPEECHER_API_KEY environment variable",
            });
        }
